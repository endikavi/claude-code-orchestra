...
export function ModeToggleButton({ instanceId }: ModeToggleButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className="fixed bottom-20 left-4 z-50 px-3 py-2 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg flex items-center gap-2 transition-all hover:scale-105 active:scale-95"
      title={`${t('modeToggle.title')} (${shortcut.label})`}
    >
      <ModeIcon className="w-5 h-5" />
      <span className="text-sm font-medium hidden sm:inline">{t('modeToggle.label')}</span>
    </button>
  );
}
